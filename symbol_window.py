...
class SymbolWindow(QDialog):
    ...
    def can_val_be_added(self):
        """ 
        Checks if the current text in valueLineEdit can be added to the working 
        list. Returns True if: 
        1) no row in the tableWidget is selected (signifying an add) 
        2) the value is different than the selected key-value pair (signifying 
          a replace)
        """
    ...
    def on_key_text_changed(self, current_text):
        ...
        tableWidget to the closest row as CURRENT_TEXT, then updates the 
        tableWidget selection, addReplaceButton, and deleteButton
        ...
        if len(self._working_list) > 0:
            # scroll to last row if current_text would be placed after the 
            # last row of tableView
        ...
        if not self.is_key_valid():
            ...
            can_add_replace = self.is_val_valid() and self.can_val_be_added()
            if found:
                self._on_row_selected(idx, can_add_replace)
            else:
                self._on_row_deselected(can_add_replace)

    def on_value_text_changed(self, current_text):
        """ 
        Called when the text in valueLineEdit is changed. Enables or disables 
        addReplaceButton depending on the values in keyLineEdit and 
        valueLineEdit. 
        """
        can_add_replace = (self.is_key_valid() and self.is_val_valid() 
            and self.can_val_be_added())
        self.ui.addReplaceButton.setEnabled(can_add_replace)

    def on_kv_return_pressed(self):
        """ 
        Called when the Enter key is pressed while either keyLineEdit or 
        valueLineEdit has focus, and behaves as if addReplaceButton was 
        clicked. If addReplaceButton cannot be clicked, do nothing.
        """
        can_add_replace = (self.is_key_valid() and self.is_val_valid() 
            and self.can_val_be_added())
